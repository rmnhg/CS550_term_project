...
void printHelp(char *binName) {
    printf("Usage: %s <IP> <port> <PathToFile> <executionEnvironment> <destFile>\n", binName);
...
int main(int argc, char **argv) {
    ...
    if (argc >= 5) {
        ...
        if (argc > 5) {
            snprintf(dest_file, strlen(argv[5]) + 1, "%s", argv[5]);
    ...
    if (status != 0 && status != 404 && (contentLength = ParseHeader(sock))) {
        int bytes = 0;
        FILE* fd;
        clock_t t;
        if (argc > 5) {
        ...
        float MiBs = ((float) contentLength)/(1024 * 1024);
        float bps = ((float) contentLength)/time_taken;
        float MiBps = ((float) contentLength)/(time_taken * 1024 * 1024);
        printf("Downloaded file of %d bytes (%.2f MiBs) in %f seconds (%.2f Bps or %.2f MiBps) in the %s environment.\n", contentLength, MiBs, time_taken, bps, MiBps, argv[4]);

        if (argc > 5)
            fclose(fd);
    } else if (status == 404) {
        printf("Error! The file was not found in the server.\n");
    } else if (status == 0) {
        printf("There was an unknown error when trying to download the file from the server.\n");
    }
